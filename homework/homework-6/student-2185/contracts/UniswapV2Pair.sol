...
contract UniswapV2Pair is IUniswapV2Pair {
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        if (_totalSupply == 0) {
            liquidity = Math.sqrt(uint(amount0) * uint(amount1)) - MINIMUM_LIQUIDITY;
           _mint(address(0), MINIMUM_LIQUIDITY);
        } else {
            liquidity = Math.min(amount0 * _totalSupply / uint(_reserve0), amount1 * _totalSupply / uint(_reserve1));
        }
        require(liquidity > 0, 'UniswapV2: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);

        _update(balance0, balance1, _reserve0, _reserve1);
        if (kLast != 0) kLast = uint(reserve0) * reserve1;
